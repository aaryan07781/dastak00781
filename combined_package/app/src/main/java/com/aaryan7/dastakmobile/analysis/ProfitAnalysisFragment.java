...
public class ProfitAnalysisFragment extends Fragment {
    ...
    public static ProfitAnalysisFragment newInstance(int period) {
        ProfitAnalysisFragment fragment = new ProfitAnalysisFragment();
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        tvTotalProfit = view.findViewById(R.id.tvTotal);
    ...
    private void loadDailyData(Date date) {
        // For daily view, we'll show hourly profit for the selected day
        repository.getDailyProfit(date, result -> {
            if (getActivity() == null) return;
            
            getActivity().runOnUiThread(() -> {
                tvTotalProfit.setText(String.format(Locale.getDefault(), "Total Profit: ₹%.2f", result));
                
                // Create sample hourly data for demonstration
                // In a real app, you would query hourly profit from the database
    ...
    private void loadWeeklyData(Date date) {
        repository.getWeeklyProfit(date, result -> {
            if (getActivity() == null) return;
    ...
    private void loadMonthlyData(Date date) {
        repository.getMonthlyProfit(date, result -> {
            if (getActivity() == null) return;
    ...
    private void loadYearlyData() {
        ...
        repository.getYearlyProfit(currentYear, result -> {
            if (getActivity() == null) return;
